...
class Book(db.Model):
    ...
    def json(self):
        ...
        #we want all our output to be in json format so it will be compatible with our flask app
        #this method we are defining will convert our output to json
        #the __repr__ works in the terminal, so we need the json method for the get_all_books() method

    def add_book(_name, _price, _isbn):
        '''function to add book to database using _name, _price, _isbn as parameters'''
        new_book = Book(name=_name, price=_price, isbn=_isbn) #creating an instance of our Book constructor
    ...
    def delete_book(_isbn):
        '''function to delete a book from our database using the isbn of the book'''
        is_successful = Book.query.filter_by(isbn=_isbn).delete() #filter by isbn and delete
    ...
    def update_book_price(_isbn, _price):
        '''function to update the price of a book using the isbn of the book and the new price of the book as parameters'''
        book_to_update = Book.query.filter_by(isbn=_isbn).first()
        book_to_update.price = _price
        db.session.commit()

    def update_book_name(_isbn, _name):
        '''function to update the name of a book using the isbn of the book and the new name of the book as parameters'''
        book_to_update = Book.query.filter_by(isbn=_isbn).first()
        book_to_update.name = _name # replace name with new name in the parameter
        db.session.commit() #commit changes

    def replace_book(_isbn, _name, _price):
        '''function to replace a book using the book's isbn, new name and new price as parameter'''
    ...
    def __repr__(self):
        ...
        #the __repr__ is a representation method, it helps us define how we want our data to be represented

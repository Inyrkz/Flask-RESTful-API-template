...
# jwt(JSON Web Token) handles authentication of API
# without client sending us username and password for every API call
...
def get_token():
    ...
    # it needs to be a string, since we will be comparing the value

    match = User.username_password_match(username, password)
    # use the username_password_match method

    if match:
        extra = datetime.timedelta(seconds=100)
        expiration_date = datetime.datetime.utcnow() + extra
        # setting the expiration date of our token
        # datetime.datetime.utcnow() is the exact time the user will access the route
...
def get_books():
    ...
    # if token is valid, then the books are displayed


# sanitizing data sent in post request
# we don't want clients to send just any data to our API
# we need to validate the data sent to the server
def validBookObject(bookObject):
    '''Function that validates the data sent to API'''
...
def add_book():
    ...
    if (validBookObject(request_data)):  # if True
        ...
        # 3 parameters, the first is the response body, second is the status code 
        # and third is the content type header that will be sent back to the client
        response.headers['Location'] = "/books/" + str(request_data['isbn'])
        # The response body is an empty string because the client won't get access 
        # to the resource from the response body but from the location header instead
...
# Note: when we return a string in flask
# it automatically sets the response type to text.html
# and the status code to 200 (successful)
...
def replace_book(isbn):
    ...
    if (not validBookObjectPut(request_data)):  # if True
        ...
        # The response body is an empty string because the client won't get access
        # to the resource from the response body but from the location header instead
        #  A location header points to where a client can go
        # and receive the new resource that was created
